...
@section('rmenu')
	<a href="{{ url('/xycms/admin/add') }}" class="btn btn-info">添加用户</a>
@endsection
@endif
	...
	@foreach($list as $m)
		<tr>
			...
			<td>
				...
				<a href="{{ url('/xycms/admin/edit',$m->id) }}" class="btn btn-sm btn-info">修改资料</a>
				@endif
				@if(App::make('com')->ifCan('admin-pwd'))
				<a href="{{ url('/xycms/admin/pwd',$m->id) }}" class="btn btn-sm btn-info">修改密码</a>
				@endif
				@if($m->id != 1 && App::make('com')->ifCan('admin-del'))
				<a href="{{ url('/xycms/admin/del',$m->id) }}" class="confirm btn btn-sm btn-danger">删除</a>
				@endif
			</td>
		</tr>
	@endforeach
