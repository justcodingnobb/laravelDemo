...
<form action="" class="pure-form pure-form-stacked" method="post">
	...
	<select name="data[type]" id="typelist">
		...
		<option value="{{ $t->val }}">{{ $t->name }}</option>
		@endforeach
	</select>
	...
	<input type="text" name="data[con]" class="pure-input-1-4" value="{{ old('data.con') }}">
    ...
    <div class="images hidden">
		<label for="mediaid" class="item-label">多媒体素材：<span class="color-9"> 回复图片消息时填写图片地址，其它消息填写微信mediaid，都可以在素材管理中找到</span></label>
		<input type="text" name="data[mediaid]" value="{{ old('data.mediaid') }}" class="pure-input-1-4">
	...
	<div class="music_div hidden">
		<label for="music_url" class="item-label">音乐链接：<span class="color-9"> 直接填写URL</span></label>
		<input type="text" name="data[music_url]" value="{{ old('data.music_url') }}" class="pure-input-1-4">
		@if ($errors->has('data.music_url'))
	        <span class="help-block">
	        	{{ $errors->first('data.music_url') }}
	        </span>
	    @endif
		<label for="hq_music_url" class="item-label">高品质音乐链接：<span class="color-9"> 直接填写URL</span></label>
		<input type="text" name="data[hq_music_url]" value="{{ old('data.hq_music_url') }}" class="pure-input-1-4">
	...
	<label for="content" class="item-label">回复内容：<span class="color-9"> 当回复普通文本时，必须填写</span></label>
	<textarea name="data[content]" id="content_edit" style="width:46%" rows="5">{{ old('data.hq_music_url') }}</textarea>
	...
	<div class="url hidden">
		<label for="url" class="item-label">回复链接：<span class="color-9"> url，必须填写</span></label>
