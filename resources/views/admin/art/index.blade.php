...
<a href="{{ url('/xycms/art/add',$catid) }}" class="btn btn-info">添加文章</a>
@endif
...
<form action="{{ url('/xycms/art/alldel') }}" class="form-inline form_status" method="get">
	{{ csrf_field() }}
	<table class="table table-striped table-hover mt10">
		...
		<tbody>
			...
			<tr>
				...
				<td>{{ $a->id }}</td>
				<td>
					<a href="{{ url('/xycms/art/show',$a->id) }}" target="_blank">{{ $a->title }}</a>
					@if($a->thumb != '')
				...
				<td>
					...
					<a href="{{ url('/xycms/art/edit',$a->id) }}" class="btn btn-sm btn-info">修改</a> 
				@endif
				@if(App::make('com')->ifCan('art-del'))
					<a href="{{ url('/xycms/art/del',$a->id) }}" class="confirm btn btn-sm btn-danger">删除</a>
					@endif
				</td>
			</tr>
			@endforeach
