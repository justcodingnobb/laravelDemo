...
<a href="{{ url('/admin/art/add',$catid) }}" class="btn btn-info">添加文章</a>
@endif
	...
	<table class="table table-striped table-hover mt10">
		...
		<tbody>
			...
			<tr>
				...
				<td>{{ $a->id }}</td>
				<td>
					<a href="{{ url('/admin/art/show',$a->id) }}" target="_blank">{{ $a->title }}</a>
					@if($a->thumb != '')
				...
				<td>
					...
					<a href="{{ url('/admin/art/edit',$a->id) }}" class="btn btn-sm btn-info">修改</a> 
				@endif
				@if(App::make('com')->ifCan('art-del'))
					<a href="{{ url('/admin/art/del',$a->id) }}" class="confirm btn btn-sm btn-danger">删除</a>
					@endif
				</td>
			</tr>
			@endforeach
