...
<div class="pull-left" data-toggle="buttons">
	...
	@if(App::make('com')->ifCan('manzeng-sort'))
	<span class="btn btn-warning btn_sort">排序</span>
	@endif

	@if(App::make('com')->ifCan('manzeng-alldel'))
...
<script>
	$(function(){
		$('.btn_sort').click(function(){
			$('.form_submit').attr({'action':"{{ url('/xyshop/manzeng/sort') }}",'method':'post'}).submit();
		});
		$('.btn_del').click(function(){
			if (!confirm("确实要删除吗?")){
				return false;
			}else{
				$('.form_submit').attr({'action':"{{ url('/xyshop/manzeng/alldel') }}",'method':'post'}).submit();
			}
		});
