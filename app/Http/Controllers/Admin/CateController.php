...
class CateController extends Controller
{
        ...
        if (is_array($tree)) {
            foreach ($tree as $v) {
                ...
                $html .= "<tr>
                    <td>".$v['listorder']."</td>
                    <td>".$v['id']."</td>
                    <td><span class='level-".$level."'></span>".$v['name']."<a href='/xycms/cate/add/".$v['id']."' class='glyphicon glyphicon-plus add_submenu'></a></td>
                    <td>".$typename."</td>
                    <td><a href='/xycms/cate/edit/".$v['id']."' class='btn btn-sm btn-info'>修改</a> <a href='/xycms/cate/del/".$v['id']."' class='confirm btn btn-sm btn-danger'>删除</a></td>
                    </tr>";
                if ($v['parentid'] != '')
                {
    ...
    public function getCache()
    {
        ...
        return redirect('/xycms/cate/index')->with('message', '更新栏目缓存成功！');
    }
    /**
     * 添加栏目
    ...
    public function postAdd(CateRequest $res,$pid = '0')
    {
        ...
        try {
            ...
            // 没出错，提交事务
            DB::commit();
            return redirect('/xycms/cate/index')->with('message', '添加成功！');
        } catch (Exception $e) {
            // 出错回滚
            DB::rollBack();
    ...
    public function postEdit(CateRequest $res,$id = '')
    {
        ...
        try {
            ...
            // 没出错，提交事务
            DB::commit();
            return redirect('/xycms/cate/index')->with('message', '修改成功！');
        } catch (Exception $e) {
            // 出错回滚
            DB::rollBack();
