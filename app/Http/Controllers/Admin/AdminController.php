...
class AdminController extends Controller
{
    ...
    public function postAdd(AdminRequest $request)
    {
        ...
        try {
            ...
            // 没出错，提交事务
            DB::commit();
            return redirect('admin/admin/index')->with('message', '添加用户成功！');
        } catch (Exception $e) {
            // 出错回滚
            DB::rollBack();
    ...
    public function postEdit(AdminRequest $request,$uid)
    {
        ...
        try {
            ...
            // 没出错，提交事务
            DB::commit();
            return redirect('admin/admin/index'.$request->input('ref'))->with('message', '修改用户成功！');
        } catch (Exception $e) {
            // 出错回滚
            DB::rollBack();
            return back()->with('message','添加失败，请稍后再试！');
    ...
    {
        $this->admin->where('id',$uid)->update(['password'=>bcrypt($request->data['password'])]);
        return redirect('admin/admin/index'.$request->input('ref'))->with('message', '修改密码成功！');
    }
    // 删除用户
    ...
    {
        $res = $this->admin->where('id',session('user')->id)->update(['password'=>bcrypt($request->data['password'])]);
        if ($res) {
