...
class MenuController extends BaseController
{
    ...
    {
        $title = '菜单列表';
        $list = $this->menu->orderBy('listorder','asc')->get();
    ...
    private function toTreeHtml($tree)
    {
        ...
        foreach ($tree as $v) {
            ...
            $disStr = $v['display'] ? "<span class='color-green'>是</span>" : "<span class='color-blue'>否</span>";
            // level < 4 是为了不添加更多的层级关系，其它地方不用判断，只是后台菜单不用那么多级
            if ($level < 4) {
                 $html .= "<tr>
                    <td>".$v['listorder']."</td>
                    <td>".$v['id']."</td>
                    <td><span class='level-".$level."'></span>".$v['name']."<div data-url='/xyshop/menu/add/".$v['id']."' class='glyphicon glyphicon-plus add_submenu btn_modal' data-title='添加菜单' data-toggle='modal' data-target='#myModal'></div></td>
                    <td>".$v['url']."</td>
                    <td>".$disStr."</td>
                    <td><div data-url='/xyshop/menu/edit/".$v['id']."' class='btn btn-sm btn-info btn_modal' data-title='修改菜单' data-toggle='modal' data-target='#myModal'>修改</div> <a href='/xyshop/menu/del/".$v['id']."' class='confirm btn btn-sm btn-danger'>删除</a></td>
                    </tr>";
            }
            else
            {
    ...
    {
        $title = '添加菜单';
    	return view('admin.menu.add',compact('pid','title','level'));
    }
    /**
     * 添加菜单提交数据
     * @param  Request $request [description]
    ...
    public function postAdd(MenuRequest $request)
    {
        ...
        return $this->ajaxReturn(1,'添加菜单成功',url('/xyshop/menu/index'));
    ...
    public function postEdit(MenuRequest $res,$id)
    {
        ...
        App::make('com')->updateCache($this->menu,'menuCache');
        return $this->ajaxReturn(1,'修改菜单成功',url('/xyshop/menu/index'));
        // return redirect('/xyshop/menu/index')->with('message', '修改菜单成功！');
    }
    /**
    ...
    public function getDel($id)
    {
        ...
        App::make('com')->updateCache($this->menu,'menuCache');
        return redirect('/xyshop/menu/index')->with('message', '删除菜单成功！');
    }
}
