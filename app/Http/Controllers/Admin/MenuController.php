...
class MenuController extends Controller
{
        ...
        foreach ($tree as $v) {
            ...
            if ($level < 4) {
                 $html .= "<tr>
                    <td>".$v['listorder']."</td>
                    <td>".$v['id']."</td>
                    <td><span class='level-".$level."'></span>".$v['name']."<a href='/admin/menu/add/".$v['id']."' class='glyphicon glyphicon-plus add_submenu'></a></td>
                    <td>".$v['url']."</td>
                    <td>".$disStr."</td>
                    <td><a href='/admin/menu/edit/".$v['id']."' class='btn btn-sm btn-info'>修改</a> <a href='/admin/menu/del/".$v['id']."' class='confirm btn btn-sm btn-danger'>删除</a></td>
                    </tr>";
            }
            else
            {
    ...
    public function postAdd(MenuRequest $request)
    {
    	...
    	return redirect('/admin/menu/index')->with('message', '添加菜单成功！');
    }
    /**
    ...
    public function postEdit(MenuRequest $res,$id)
    {
        ...
        App::make('com')->updateCache($this->menu,'menuCache');
        return redirect('/admin/menu/index')->with('message', '修改菜单成功！');
    }
    /**
    ...
    public function getDel($id)
    {
        ...
        App::make('com')->updateCache($this->menu,'menuCache');
        return redirect('/admin/menu/index')->with('message', '删除菜单成功！');
    }
}
