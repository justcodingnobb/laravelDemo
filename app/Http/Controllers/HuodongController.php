...
class HuodongController extends BaseController
{
    ...
    public function getIndex()
    {
    	...
    	$list = Huodong::where('starttime','<',date('Y-m-d H:i:s'))->where('endtime','>',date('Y-m-d H:i:s'))->where('status',1)->where('del',1)->orderBy('sort','asc')->orderBy('id','desc')->simplePaginate(20);
        $info->pid = 0;
