...
use App\Http\Controllers\Push\HttpRequest;
use App\Http\Controllers\Push\PushSimpleLog;
use App\Http\Controllers\Push\AssertHelper;
...
class BaseSDK{
    ...
    const ERR_CLIENT = 'App\Http\Controllers\Push\ClientException';
    const ERR_NET = 'App\Http\Controllers\Push\HttpException';
    const ERR_SERVER = 'App\Http\Controllers\Push\ServerException';
    ...
    function __construct($curlOpts = array()) {
        ...
        self::$suppressException = config('push.SUPPRESS_EXCPTION');
        $this -> log = new PushSimpleLog(config('push.LOG_OUTPUT'), config('push.LOG_LEVEL_TRACE'));
        
        try {
            
            $this -> http = new HttpRequest(config('push.host'), $curlOpts, $this -> log);
        ...
        $this -> signExpires = intval(config('push.SIGN_EXPIRES'));
    ...
    public function newApiQueryTpl() {
        ...
        if ($this -> deviceType === null && config('push.default_devicetype') !== null) {
            $this->setDeviceType(config('push.default_devicetype'));
        }
